...
;	Global @ScriptDir = IniRead($sIniFile, "Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", $iniCheck)
...
;		$UpdateInterval = 5
	...
	ElseIf $g_sUseInGameUpdate = "yes" And $g_sRconEnabled <> "yes" Then
		...
		FileWriteLine($sLogFile, _NowCalc() & " Server RCON is Disabled. Disabling MCRCON Notifications. Cannot send RCON message without RCON enabled!")
	EndIf
;	If $iniCheck = @ScriptDir Then
;		@ScriptDir = "D:\Game Servers\Conan Exiles Dedicated Server\mcrcon"
;		$iIniFail += 1
;	EndIf
...
;	IniWrite($sIniFile, "Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", @ScriptDir)
