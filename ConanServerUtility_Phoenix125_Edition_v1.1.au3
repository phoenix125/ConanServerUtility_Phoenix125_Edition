...
#AutoIt3Wrapper_Outfile=builds\ConanServerUtility_Phoenix125_Edition_v1.1.exe
#AutoIt3Wrapper_Outfile_x64=builds\ConanServerUtility_Phoenix125_Edition_v1.1.exe
...
Global $DeveloperNotes = 1  ; Enabling this (1) will add log entry for each function executed. For troubleshooting purposes only.
Global $aBotMsg = "Server restart required. "
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $g_sUseMCRCONScheduled = IniRead($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconSCHEDULED", $iniCheck)
	Global $g_sUseMCRCONUpdate = IniRead($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconUPDATE", $iniCheck)
	Global $g_sMCrconPath = IniRead($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", $iniCheck)
...
;		$UpdateInterval = 5
	...
	If $iniCheck = $g_sMCrconPath Then
		$g_sMCrconPath = "D:\Game Servers\Conan Exiles Dedicated Server\mcrcon"
		$iIniFail += 1
	EndIf
	...
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") Then
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconSCHEDULED", $g_sUseMCRCONScheduled)
	IniWrite($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "use_mcrconUPDATE", $g_sUseMCRCONUpdate)
	IniWrite($sIniFile, "Use MCRCON to Send In Game Message to Announce Scheduled/Update Restarts? (yes/no)", "mcrconPath", $g_sMCrconPath)
...
Func Gamercide()
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) Gamercide")
EndIf
...
Func CloseServer()
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) CloseServer")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) CheckRange")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) CheckSetting")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) RaidCheck")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) AvatarCheck")
EndIf
	Local $sServerSettingsFile = $serverdir & $g_sServerSettingIniLoc
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) GetLatestVersion")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) GetInstalledVersion")
EndIf
	Local $aReturn[2] = [False, ""]
	...
	EndIf
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) END GetInstalledVersion")
EndIf
	Return $aReturn
...
Func UpdateCheck()
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) UpdateCheck")
EndIf
	...
	If ($aLatestVersion[0] And $aInstalledVersion[0]) Then
		...
		Else
			...
			Global $aBotMsg = $sAnnounceServerUpdateMessage
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) Functions to Check for Mod " & $sMod & " Updates")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) GetInstalledModUpdateTime")
EndIf
	Local $aReturn[3] = [False, False, ""]
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) CheckMod")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) WriteModList")
EndIf
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) UpdateModNameList")
EndIf
FileWriteLine($g_c_sLogFile, "TESTING: UpdateModNameList")
...
If $DeveloperNotes = 1 Then
	FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (DEVELOPER NOTES: Executing Region/Funct:) UpdateMod")
EndIf
	Local $bReturn = False
...
If ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") Then
...
While True ;**** Loop Until Closed ****
	...
	If ($CheckForUpdate = "yes") And ((_DateDiff('n', $g_sTimeCheck0, _NowCalc())) >= $UpdateInterval) And ($g_iBeginDelayedShutdown = 0) Then
		...
		If $bRestart And (($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes")) Then
			$g_iBeginDelayedShutdown = 1
	...
	If ($sUseDiscordBotScheduled = "yes") Or ($sUseDiscordBotUpdate = "yes") Or ($sUseTwitchBotScheduled = "yes") Or ($sUseTwitchBotUpdate = "yes") Or ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") Then
		If $g_iBeginDelayedShutdown = 1 Then
			...
			Local $sShutdownMessage = $ServerName & ": " & $aBotMsg & " Restarting in " & $g_iDelayShutdownTime & " minutes"
			...
			If $aBotMsg = $sAnnounceScheduledMessage Then
				If $g_sUseMCRCONScheduled = "yes" Then
					MCRCONcmd($g_sMCrconPath, $g_IP, $g_iRconPort, $g_sRconPass, "broadcast", $sInGameMsgDaily)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_sConanPID & ")] [RCON] Sent ( " & $sInGameMsgDaily & " ) to Players")
				EndIf
				If $sUseDiscordBotScheduled = "yes" Then
			...
			If $aBotMsg = $sAnnounceServerUpdateMessage Then
				If $g_sUseMCRCONUpdate = "yes" Then
					MCRCONcmd($g_sMCrconPath, $g_IP, $g_iRconPort, $g_sRconPass, "broadcast", $sInGameMsgUpdate)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_sConanPID & ")] [RCON] Sent ( " & $sInGameMsgUpdate & " ) to Players")
				EndIf
			...
			If $aBotMsg = $sAnnounceModUpdateMessage Then
				If $g_sUseMCRCONUpdate = "yes" Then
					MCRCONcmd($g_sMCrconPath, $g_IP, $g_iRconPort, $g_sRconPass, "broadcast", $sInGameMsgModUpdate)
					FileWriteLine($g_c_sLogFile, _NowCalc() & " [" & $ServerName & " (PID: " & $g_sConanPID & ")] [RCON] Sent ( " & $sInGameMsgModUpdate & " ) to Players")
				EndIf
				If $sUseDiscordBotUpdate = "yes" Then
					SendDiscordMsg($sDiscordWebHookURLs, $sShutdownMessage, $sDiscordBotName, $bDiscordBotUseTTS, $sDiscordBotAvatar)
				EndIf
