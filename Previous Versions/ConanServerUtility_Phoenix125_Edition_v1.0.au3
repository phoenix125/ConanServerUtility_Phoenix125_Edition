...
#AutoIt3Wrapper_Outfile=builds\ConanServerUtility_Phoenix125_Edition_v1.0.exe
#AutoIt3Wrapper_Outfile_x64=builds\ConanServerUtility_x64_v3.3.0-beta.3.exe
...
Func ReadUini($sIniFile, $sLogFile)
	...
	Global $sAnnounceUpdateMessage = IniRead($sIniFile, "Message to Announce Server Restarts on Discord and Twitch (if enabled)?", "AnnounceUPDATEMessage", $iniCheck)
...
Func UpdateIni($sIniFile)
	...
	IniWrite($sIniFile, "Message to Announce Server Restarts on Discord and Twitch (if enabled)?", "AnnounceUpdateMessage", $sAnnounceUpdateMessage)
...
If ($g_sUseMCRCONScheduled = "yes") Or ($g_sUseMCRCONUpdate = "yes") Then
	...
	If $sFileExists = 0 Then
		FileWriteLine($g_c_sLogFile, _NowCalc() & " Downloading and installed mcrcon.exe")
