...
#AutoIt3Wrapper_Outfile=builds\ConanServerUtility_Phoenix125_Edition_v1.3.5.exe
#AutoIt3Wrapper_Outfile_x64=builds\ConanServerUtility_Phoenix125_Edition_v1.3.5.exe
...
While True ;**** Loop Until Closed ****
	...
	If Not ProcessExists($g_sConanPID) Then
		...
		If $validategame = "yes" Or $aValidate = "yes" Then
			Global $aValidate = "no"
			If $g_sSteamLoginName = "" Then
				Local $tCMD3 = "" & $steamcmddir & "\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir """ & $serverdir & """ +app_update 443030 validate +quit"
			Else
				Local $tCMD3 = "" & $steamcmddir & "\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login " & $g_sSteamLoginName & " " & $g_sSteamLoginPassword & " +force_install_dir """ & $serverdir & """ +app_update 443030 validate +quit"
			EndIf
			If $g_bDebug Then
		...
		Else
			If $g_sSteamLoginName = "" Then
				Local $tCMD4 = "" & $steamcmddir & "\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +force_install_dir """ & $serverdir & """ +app_update 443030 +quit"
			Else
				Local $tCMD4 = "" & $steamcmddir & "\steamcmd.exe +@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login " & $g_sSteamLoginName & " " & $g_sSteamLoginPassword & " +force_install_dir """ & $serverdir & """ +app_update 443030 +quit"
			EndIf
			If $g_bDebug Then
